...
from agenda_api import router as agenda_router

load_dotenv()

logger = logging.getLogger("totvs")
...
def add_cond(where_clause: str, cond: str) -> str:
    """
    Adiciona uma condição extra ao WHERE sem quebrar a precedência de OR.
    Envolve as condições existentes em parênteses antes de aplicar o AND.
    """
    if where_clause == "WHERE 1=1":
        return f"WHERE {cond}"
    return "WHERE (" + where_clause.removeprefix("WHERE ") + ") AND " + cond
...
async def estatisticas_gerais(search: str = Query("")):
    ...
    total_reunioes_res = await sync_run_one(
        f"SELECT COUNT(DISTINCT ID_MEETING) as total_reunioes FROM reunioes {where_clause}",
        params,
    )
    stats["total_reunioes"] = total_reunioes_res["total_reunioes"] if total_reunioes_res else 0

    trans_where = add_cond(
        where_clause, "ANON_TRANSCRICAO IS NOT NULL AND ANON_TRANSCRICAO != ''"
    )
    total_trans_res = await sync_run_one(
        f"SELECT COUNT(*) as total FROM reunioes {trans_where}", params
    )
    stats["total_transcricoes"] = total_trans_res["total"] if total_trans_res else 0

    duracao_media_res = await sync_run_one(
        f"SELECT AVG({DURACAO_MIN}) as duracao_media_minutos FROM reunioes {where_clause}",
        params,
    )
    stats["duracao_media_minutos"] = float(duracao_media_res["duracao_media_minutos"] or 0)

    clientes_res = await sync_run_one(
        f"SELECT COUNT(DISTINCT CODT) as total_clientes FROM reunioes {where_clause}", params
    )
    stats["total_clientes"] = clientes_res["total_clientes"] if clientes_res else 0

    stats["reunioes_por_formato"] = await sync_run_query(
        f"SELECT FORMATO_MEETING, COUNT(DISTINCT ID_MEETING) as count FROM reunioes {where_clause} GROUP BY FORMATO_MEETING",
        params,
    )
    stats["reunioes_por_status"] = await sync_run_query(
        f"SELECT STATUS_MEETING, COUNT(DISTINCT ID_MEETING) as count FROM reunioes {where_clause} GROUP BY STATUS_MEETING",
        params,
    )
    stats["reunioes_por_mes"] = await sync_run_query(
        f"SELECT substr(DT_MEETING,1,7) as mes, COUNT(DISTINCT ID_MEETING) as count FROM reunioes {where_clause} GROUP BY mes ORDER BY mes DESC LIMIT 12",
        params,
    )

    nps_res = await sync_run_one(
        f"SELECT AVG(NOTA_NPS) as nps_medio FROM reunioes {where_clause}", params
    )
    stats["nps_medio"] = float(nps_res["nps_medio"] or 0)

    where_uf = add_cond(where_clause, "UF IS NOT NULL")
    stats["reunioes_por_uf"] = await sync_run_query(
        f"SELECT UF, COUNT(DISTINCT ID_MEETING) as count FROM reunioes "
        f"{where_uf} GROUP BY UF ORDER BY count DESC",
        params,
    )

    unidades_res = await sync_run_one(
        f"SELECT COUNT(DISTINCT NOME_UNIDADE) as total_unidades FROM reunioes {where_clause}",
        params,
    )
    stats["total_unidades"] = unidades_res["total_unidades"] if unidades_res else 0

    stats["segmentos_mais_comuns"] = await sync_run_query(
        f"SELECT NOME_SEGMENTO, COUNT(DISTINCT ID_MEETING) as count FROM reunioes {where_clause} GROUP BY NOME_SEGMENTO ORDER BY count DESC LIMIT 10",
        params,
    )
    stats["faixa_faturamento_count"] = await sync_run_query(
        f"SELECT FAIXA_FATURAMENTO_CLIENTE_EC as faixa, COUNT(DISTINCT ID_MEETING) as count FROM reunioes {where_clause} GROUP BY FAIXA_FATURAMENTO_CLIENTE_EC",
        params,
    )
    stats["distribuicao_nps"] = await sync_run_query(
        f"""
        SELECT CASE
...
async def analises_completas(search: str = Query("")):
    ...
    cond_uf = "UF IS NOT NULL AND UF != ''"
    where_uf = add_cond(where_clause, cond_uf)
    stats["por_uf"] = await sync_run_query(
        f"SELECT UF, COUNT(DISTINCT ID_MEETING) as count FROM reunioes "
        f"{where_uf} GROUP BY UF ORDER BY count DESC",
        params,
    )

    cond_seg = "NOME_SEGMENTO IS NOT NULL AND NOME_SEGMENTO != ''"
    where_seg = add_cond(where_clause, cond_seg)
    stats["por_segmento"] = await sync_run_query(
        f"SELECT NOME_SEGMENTO, COUNT(DISTINCT ID_MEETING) as count FROM reunioes "
        f"{where_seg} GROUP BY NOME_SEGMENTO ORDER BY count DESC LIMIT 10",
        params,
    )

    stats["por_mes"] = await sync_run_query(
        f"SELECT substr(DT_MEETING,1,7) as mes, COUNT(DISTINCT ID_MEETING) as count FROM reunioes {where_clause} GROUP BY mes ORDER BY mes DESC LIMIT 12",
        params,
    )

    cond_fmt = "FORMATO_MEETING IS NOT NULL AND FORMATO_MEETING != ''"
    where_fmt = add_cond(where_clause, cond_fmt)
    stats["por_formato"] = await sync_run_query(
        f"SELECT FORMATO_MEETING, COUNT(DISTINCT ID_MEETING) as count FROM reunioes "
        f"{where_fmt} GROUP BY FORMATO_MEETING",
        params,
    )

    cond_status = "STATUS_MEETING IS NOT NULL AND STATUS_MEETING != ''"
    where_status = add_cond(where_clause, cond_status)
    stats["por_status"] = await sync_run_query(
        f"SELECT STATUS_MEETING, COUNT(DISTINCT ID_MEETING) as count FROM reunioes "
        f"{where_status} GROUP BY STATUS_MEETING",
        params,
    )

    cond_faixa = "FAIXA_FATURAMENTO_CLIENTE_EC IS NOT NULL AND FAIXA_FATURAMENTO_CLIENTE_EC != ''"
    where_faixa = add_cond(where_clause, cond_faixa)
    stats["por_faturamento"] = await sync_run_query(
        f"SELECT FAIXA_FATURAMENTO_CLIENTE_EC as faixa, COUNT(DISTINCT ID_MEETING) as count FROM reunioes "
        f"{where_faixa} GROUP BY faixa ORDER BY count DESC LIMIT 10",
        params,
    )
    ...
    cond_unid = "NOME_UNIDADE IS NOT NULL AND NOME_UNIDADE != ''"
    where_unid = add_cond(where_clause, cond_unid)
    stats["top_unidades"] = await sync_run_query(
        f"SELECT NOME_UNIDADE, COUNT(DISTINCT ID_MEETING) as count FROM reunioes "
        f"{where_unid} GROUP BY NOME_UNIDADE ORDER BY count DESC LIMIT 10",
        params,
    )

    cond_dur_seg = "NOME_SEGMENTO IS NOT NULL AND NOME_SEGMENTO != ''"
    where_dur_seg = add_cond(where_clause, cond_dur_seg)
    stats["duracao_por_segmento"] = await sync_run_query(
        f"SELECT NOME_SEGMENTO, ROUND(AVG({DURACAO_MIN}), 1) as duracao_media, COUNT(DISTINCT ID_MEETING) as qtd FROM reunioes "
        f"{where_dur_seg} GROUP BY NOME_SEGMENTO HAVING qtd >= 3 ORDER BY duracao_media DESC LIMIT 10",
        params,
    )
    return stats
...
async def assistente_ask(req: AssistenteRequest):
    ...
    logger.info("PERGUNTA: %s", req.question)

    # ===== 1. ESTATÍSTICAS GERAIS (agregadas no SQL) =====
...
3. Seja direto e específico, cite números e nomes"""
        ...
        try:
            ...
            logger.exception("Falha na chamada à API de IA — usando fallback local")
